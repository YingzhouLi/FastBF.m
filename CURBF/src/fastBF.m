function [Factor,Rcomp] = fastBF(fun_org,xx,kk,r,tol,tag)
...
levels = 0;%max(0,min(floor(log2([npx npk])-3)));
Dim = size(xx,2);
...
for ell = 1:levels
    ...
    for itx = 1:prod(npxx)
        ...
        xxparidx = parentidx(npxx,itx);
        for itk = 1:prod(npkk)
            cursk = [];
            for itk_child = childidx(npkk,itk)
                cursk = [cursk; skpre{xxparidx,itk_child}];
            end
            [ UR, sk{itx,itk} ] = CURR(fun, xxval, cursk, r);
            offset = 0;
            for itk_child = childidx(npkk,itk)
...
for ell = 1:levels
    ...
    for itk = 1:prod(npkk)
        ...
        kkparidx = parentidx(npkk,itk);
        for itx = 1:prod(npxx)
            cursx = [];
            for itx_child = childidx(npxx,itx)
                cursx = [cursx; sxpre{kkparidx,itx_child}];
            end
            [ CU, sx{itk,itx} ] = CURL(fun, cursx, kkval, r);
            offset = 0;
            for itx_child = childidx(npxx,itx)
